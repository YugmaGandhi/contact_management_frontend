...
const AllContact = () => {
  ...
  useEffect(() => {
    async function fetchData(){
      setLoading(true);
    try {
      const res = await fetch(`https://contact-management-backend-4ivh.onrender.com/api/mycontacts`, {
        method: "GET",
        headers: {
  ...
  const deleteContact = async (id) => {
    if (window.confirm("are you sure you want to delete this contact ?")) {
      try {
        const res = await fetch(`https://contact-management-backend-4ivh.onrender.com/api/delete/${id}`, {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
