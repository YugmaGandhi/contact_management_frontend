...
const EditContact = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    const res = await fetch(`https://contact-management-backend-4ivh.onrender.com/api/contact`, {
      method: "PUT",
      headers: {
  ...
  useEffect( () => {
    async function fetchData(){
      setLoading(true);
      try {
        const res = await fetch(`https://contact-management-backend-4ivh.onrender.com/api/contact/${id}`, {
          method: "GET",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
